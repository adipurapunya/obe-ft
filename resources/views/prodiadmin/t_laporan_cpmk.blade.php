...
<div class="content-header">
        <div class="container-fluid">
            <h1 class="m-0 text-center">Laporan Capaian CPMK, CPL, dan SUB CPL Mahasiswa</h1>
        </div>
    </div>
                ...
                <div class="card-body">
                    <form id="form-filter">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <label><strong>Masukkan NIM</strong></label>
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header bg-success text-white">
                            <h5 class="m-0">Rata-Rata Nilai Sub-CPL</h5>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header bg-info text-white">
                            <h5 class="m-0"><span id="judulGrafik">Grafik Pencapaian Sub-CPL</span></h5>
                        </div>
                        <div class="card-body">
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header bg-secondary text-white">
                            <h5 class="m-0"><span id="judulGrafikCPL">Grafik Pencapaian CPL</span></h5>
                        </div>
                        <div class="card-body">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-filter').click(function() {
                ...
                if (!kurikulumId || !nim || !semesterId) {
                    Swal.fire({ icon: 'warning', title: 'Oops...', text: 'Harap pilih Kurikulum, semester dan masukkan NIM!' });
                    return;
                }
                ...
                $.ajax({
                    url: "{{ url('/prodiadmin/getCapaianCPL') }}",
                    type: "GET",
                    data: { kurikulum_id: kurikulumId, nim: nim, semester_id: semesterId },
                    success: function(response) {
                        ...
                        if (response.nilai_keseluruhan.length > 0) {
                            ...
                            // Update Info Mahasiswa di halaman
                            $('#nimDisplay').text(nim);
                            $('#namaMahasiswaDisplay').text(nama_mhs);
                            $('#mahasiswaInfo').fadeIn(); // Tampilkan div info mahasiswa
                            
                            if(response.sem == 'Semua Semester'){
                                $("#judulGrafik").text(`Grafik Pencapaian Sub-CPL - ${nama_mhs} - ${nim} - (Semua Semester)`);
                                $("#judulGrafikCPL").text(`Grafik Pencapaian CPL - ${nama_mhs} - ${nim} - (Semua Semester)`);
                            }else{
            ...
            function getColumns(tableId) {
                return {
                    '#nilai_keseluruhan': [
                        ...
                        { data: 'KODE_SUB_CPL', className: 'text-center' }, { data: 'NILAI_MHS', className: 'text-center' }, { data: 'ID_SEMESTER', className: 'text-center' }, { data: 'STATUS', className: 'text-center' }
                    ],
                    '#rata_subcpl': [
                        { data: null, render: (data, type, row, meta) => meta.row + 1, className: 'text-center' },
            ...
            function updateChart(data) {
                ...
                subcplChart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            label: 'Nilai Rata-Rata Sub-CPL Per semester',
                    ...
                    options: {
                        ...
                        scales: {
                            y: {
                                beginAtZero: true, // Paksa agar Y selalu mulai dari 0
                                min: 0, // Pastikan minimum tetap 0
                                max: 100, // Pastikan maksimum tetap 100
                                ticks: {
                                    stepSize: 10, // Interval angka di sumbu Y (0, 10, 20, ..., 100)
                                    callback: function(value) {
                                        return value.toFixed(0) + '%'; // Pastikan nilai ditampilkan sebagai integer + %
                                    }
                                }
                            }
                        },
                        animation: {
                            duration: 0 // Nonaktifkan animasi agar nilai langsung terlihat
                        }
                    }
                });
            }
